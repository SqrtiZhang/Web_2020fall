...
BASE_DIR = '../data/Sqrti/glove_42B_300d'
...
if __name__ == "__main__":
    embeddings_index = {}
    with open(os.path.join(GLOVE_DIR, 'glove.42B.300d.txt'), encoding='utf-8') as f:  # read pre-trained word embedding
        for line in f:
            values = line.split()
            word = values[0]
    ...
    Y_train = []
    label_to_y = dict()
    with open(os.path.join(GLOVE_DIR,"train.txt")) as f:
    ...
    X_test = []
    ID_test = []
    with open(os.path.join(GLOVE_DIR,"test.txt")) as f:
        for l in f:
